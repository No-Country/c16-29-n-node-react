...
export const userRegisterValidationRules = () => {
  return checkExact([
    ...
    body("role")
      .isString()
      .withMessage("Debe ser un string")
      .custom((value) => ["PRINCIPAL", "TEACHER", "TUTOR", "STUDENT"].includes(value))
      .withMessage("Debe ser un rol definido (\"PRINCIPAL\" | \"TEACHER\" | \"TUTOR\") | \"STUDENT\""),
    body("grade")
  ...
  return checkExact([
    ...
    body("email")
      ...
      .custom(async (value, { req }) => {
        ...
        if(user.role !== "TEACHER") throw new Error("Las materias es solo para el rol de profesor")
      })
      .withMessage("Las materias es solo para el rol de profesor")
      .isArray()
      .withMessage("Debe ser un arreglo"),
      ...
      .custom(async (value, { req }) => {
        ...
        const user = await UserModel.findByPk(id)
        if(!user) throw new Error("No se encontró el usuario")
        if(user.role !== "STUDENT") throw new Error("Los tutores solo son para el rol de estudiante")
      })
      .withMessage("Los tutores solo son para el rol de estudiante")
      .isArray()
      .withMessage("Debe ser un arreglo"),
    body("students")
      ...
      .custom(async (value, { req }) => {
        ...
        const user = await UserModel.findByPk(id)
        if(!user) throw new Error("No se encontró el usuario")
        if(user.role !== "TUTOR") throw new Error("Los estudiantes solo son para el rol de tutores")
      })
      .withMessage("Los estudiantes solo son para el rol de tutores")
      .isArray()
      .withMessage("Debe ser un arreglo")
